package team169_baseline.navigation;

import team169_baseline.managers.*;
import team169_baseline.robots.*;
import team169_baseline.util.*;
import battlecode.common.*;

public class NavigationSystem {
    private final double MAP_UGLINESS_WEIGHT=1.5;
    ...
    public void setDestination(MapLocation destination) {
        ...
        expectedMovesToReachTarget = (int)(Math.sqrt(br.curLoc.distanceSquaredTo(destination)) *
                MAP_UGLINESS_WEIGHT)+10;
        this.destination = destination;
        normalBug.setTarget(destination.x, destination.y);
...
//          }
   ...
   /* private Direction navigateTangentBug() {
    ...
    public Direction navigateCompletelyRandomly() {
        ...
        int b = rand%2*2+3;
        for(int i=0; i<8; i++) {
            Direction dir = Direction.values()[(a+i*b)%8];
            if(!mapCache.isVoid(br.curLoc.add(dir)))
                return dir;
        }
    ...
    public Direction navigateRandomly(MapLocation destination) {
        ...
        if(d<0) {
            do {
                d++;
                dir = dir.rotateLeft();
            } while(d<0 || mapCache.isVoid(br.curLoc.add(dir)));
        } else {
            do {
                d++;
                dir = dir.rotateRight();
    ...
    public Direction navigateGreedy(MapLocation destination) {
        ...
        if(Util.randDouble()<0.5) {
            while(mapCache.isVoid(br.curLoc.add(dir)))
                dir = dir.rotateLeft();
        } else {
            while(mapCache.isVoid(br.curLoc.add(dir)))
                dir = dir.rotateRight();
    ...
    public static Direction getRandomDirection() {
        return Direction.values()[(int)(Util.randDouble()*8)];
    }
}